...
export function patchConditions() {
  CONFIG.statusEffects = CONDITION_ICONS.map((icon) => `modules/obsidian/img/conditions/${icon}.svg`)
    ...
    return function () {
      if (this.actor && this.actor.data.obsidian) {
        this.data.effects = Object.entries(this.actor.data.obsidian.conditions)
          ...
          .concat(
            this.data.effects.filter((effect) => CONDITION_ICONS.includes(/\/([^./]+)\./.exec(effect)[1]) === false)
          )
      }
