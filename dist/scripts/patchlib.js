    ...
    static patchClass(klass, func, line_number, line, new_line) {
      ...
      if (lines[line_number].trim() == line.trim()) {
        ...
        if (klass !== undefined) {
          ...
          if (!(fixed.startsWith('function') || fixed.startsWith('async function'))) fixed = `function ${fixed}`
        ...
        console.log(
          'Cannot patch function. It has wrong content at line ',
          line_number,
          ' : ',
          lines[line_number].trim(),
          ' != ',
          line.trim(),
          '\n',
          funcStr
