(function () {
  ...
  function onUpdateActor(actor, updated) {
    ...
    if (!get_1(updated, 'data.resources')) return;
    ...
    Object.entries(updated.data.resources).map(function (_ref3) {var _ref4 = slicedToArray(_ref3, 2),label = _ref4[0],resource = _ref4[1];
      ...
      var max = get_1(resource, 'max') !== undefined ? get_1(resource, 'max') : currentResource.max;
      var value = get_1(resource, 'value') !== undefined ? get_1(resource, 'value') : currentResource.value;
  ...
  function onUpdateOwnedItem(actor, item) {var _actor$update;
    // get resource name
    var obsidianResourceName = get_1(item, 'obsidian.bestResource.label');
    ...
    if (!label) return;
    if (
    resource.max == get_1(item, 'obsidian.bestResource.max') &&
    resource.value == get_1(item, 'obsidian.bestResource.remaining'))

    return;

    actor.update((_actor$update = {}, defineProperty(_actor$update, "data.resources.".concat(
    label, ".max"), get_1(item, 'obsidian.bestResource.max')), defineProperty(_actor$update, "data.resources.".concat(
    label, ".value"), get_1(item, 'obsidian.bestResource.remaining')), _actor$update));
  ...
  function onUpdateDefaultAC(actor, updated) {
    ...
    if (!get_1(updated, 'data.attributes.ac')) return;

    // console.log('HOOKS >', 'updateActor >', 'updateAC', { actor, updated })

    // get correspondent obsidian ac
    var O_base = get_1(actor, 'data.flags.obsidian.attributes.ac.base', 10);
    var O_mod1 = get_1(actor.data, "data.abilities.".concat(get_1(actor.data, 'flags.obsidian.attributes.ac.ability1'), ".mod"), 0);
    var O_mod2 = get_1(actor.data, "data.abilities.".concat(get_1(actor.data, 'flags.obsidian.attributes.ac.ability2'), ".mod"), 0);

    var newAc = get_1(updated, 'data.attributes.ac.value', 10 + O_mod1 + O_mod2);
  ...
  function onUpdatedObsidianAC(actor, updated) {
    // if not a ac update, bail out
    if (!get_1(updated, 'flags.obsidian.attributes.ac')) return;

    // console.log('HOOKS >', 'updateActor >', 'updateObsidianAC', { actor, updated })

    // get correspondent default ac
    var acValue = get_1(actor, 'data.data.attributes.ac.value', 10);

    var updatedAc = get_1(updated, 'flags.obsidian.attributes.ac');
    var obsidianAc = get_1(actor, 'data.flags.obsidian.attributes.ac');

    var newBase = get_1(updatedAc, 'base', get_1(obsidianAc, 'base', 10));
    var newAbility1 = get_1(updatedAc, 'ability1', get_1(obsidianAc, 'ability1'));
    var newAbility2 = get_1(updatedAc, 'ability2', get_1(obsidianAc, 'ability2'));

    var O_mod1 = get_1(actor.data, "data.abilities.".concat(newAbility1, ".mod"), 0);
    var O_mod2 = get_1(actor.data, "data.abilities.".concat(newAbility2, ".mod"), 0);
